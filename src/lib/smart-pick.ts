...
function showNotice(text: string, type: NoticeType = "info") {
  ...
  style.textContent = `
    @keyframes pick-shell-fadeInDown {
      from { opacity: 0; transform: translate(-50%, -20px); }
      to { opacity: 1; transform: translate(-50%, 0); }
    }
    @keyframes pick-shell-fadeOutUp {
  ...
  notice.style.cssText =
    ...
    "animation:pick-shell-fadeInDown 0.3s ease;pointer-events:none;" +
  ...
  setTimeout(() => {
    notice.style.animation = "pick-shell-fadeOutUp 0.3s ease"
    setTimeout(() => {
      notice.remove()
      style.remove()
    }, 300)
